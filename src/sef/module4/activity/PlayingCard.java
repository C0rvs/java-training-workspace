...
public class PlayingCard {
	...
	public PlayingCard(int number, int suit) {
		...
		if(number > KING || suit > SPADES || number < ACE || suit < CLUBS) {
			this.number = JOKER;
			this.suit = JOKER;
			}
		else {
	...
	public String toString() {
		...
		String valueOfNumber = arrNumbers[this.number];
		String valueOfSuit = arrSuits[this.suit]; 
		if(suit != JOKER || number != JOKER)
			return String.format("%s of %s", valueOfNumber, valueOfSuit);
		return "JOKER";
